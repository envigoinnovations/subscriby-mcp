...
- **Finding a member by account.** `find_member_by_identity` turns the id a bot or a server hands over into a Subscriby member: `project_id`, the `connector` key and the platform's own `external_id` answer the same row as `get_subscriber`, with `identities` loaded; an account nobody in the project linked is `RESOURCE_NOT_FOUND`, indistinguishable from an unknown project. Its REST twin is the `identity` filter on `GET /v1/projects/{project}/members`. Taking the server to 161 tools.

- **Restoring access after a reinstall.** `restore_connector_access` asks a reinstalled connector about every place the uninstall detached, reactivates the ones it still controls and hands every live purchase of a plan granting them fresh access (`member.resource_added` per grant issued); places the connector no longer controls stay detached for `run_connector_doctor` to explain, and plans taken off sale stay off sale.
...
- **The bot-family names.** `get_bot_status` and `disconnect_bot` are gone; `get_connector_installation` and `disconnect_connector` are the tools. `create_project` and `update_project` no longer take `type`: a project has no platform of its own and gains one by installing a connector. Resource rows carry `kind`, `connector` and `space` in place of `type` and `chat_id`, and `list_resources` narrows with `kind` and `connector` instead of `type`. Subscriber rows carry `identities`, one per connected account, in place of `chat_id`. Installation and resource health reasons read `connector_api_unauthorized`, `connector_api_not_found`, `connector_api_rate_limited` and `connector_api_error` in place of their `telegram_api_*` spellings. Agents and scripts sending the old names must be updated. The server stands at 161 tools and 6 resources.

### Changed

- **The listing speaks to paid communities on connected platforms.** The registry description no longer names one platform: Subscriby manages projects, plans, members, payments, connectors and analytics for creators who run paid communities wherever they live.

- **The server speaks connectors.** The server instructions introduce Subscriby as a connector-based community monetisation SaaS, and every tool description names the connector rather than one platform: broadcasts go through the project's connector, bans and kicks run the connector removal pipeline, gated places are linked through the connector with `request_resource_link`, replies and reminders reach the member on their connector. `get_bot_status` and `disconnect_bot` describe themselves as the deprecated aliases of `get_connector_installation` and `disconnect_connector` for the first connector's bot, and the `bot_id` argument of `create_project` and `update_project` is marked deprecated in favour of `install_connector`. Wire names and payloads are unchanged.

- **Payment method rows carry a provider key and its connector.** `list_payment_methods`, `get_payment_method`, `activate_payment_method` and `deactivate_payment_method` now return `provider` as the stored key — a gateway code such as `stripe`, or `connector:provider` such as `telegram:stars` for a currency a connector brings — plus a `connector` field naming that connector (`null` for gateways and access codes). The `subscriby://enums/payment-provider` resource lists the same keys with `connector`, `requires_connector` and `legacy_alias`; `platformcurrency` is the legacy alias earlier native rows carried. A project may hold one method per provider key and mode, so two connectors can each bring their own currency.
